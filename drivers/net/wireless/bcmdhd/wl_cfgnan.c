...
wl_cfgnan_set_vars_cbfn(void *ctx, void **tlv_buf, uint16 type, uint16 len)
{
	...
	case WL_NAN_XTLV_PEER_INSTANCE_ID:
		ret = bcm_unpack_xtlv_entry(tlv_buf,
			WL_NAN_XTLV_PEER_INSTANCE_ID, sizeof(wl_nan_instance_id_t),
			&ndata->peer_inst_id);
		break;
...
int wl_cfgnan_p2p_ie_enable_handler(struct net_device *ndev,
	struct bcm_cfg80211 *cfg, char* buf, nan_cmd_data_t *cmd_data) {
	int res = BCME_OK;
	...
	struct bcm_cfg80211 *cfg, char *cmd, nan_cmd_data_t *cmd_data)
{
	...
	ret = bcm_pack_xtlv_entry(&pxtlv, &end, WL_NAN_XTLV_INSTANCE_ID,
		sizeof(wl_nan_instance_id_t), &cmd_data->local_id);
	if (unlikely(ret)) {
		goto fail;
	}
	ret = bcm_pack_xtlv_entry(&pxtlv, &end, WL_NAN_XTLV_REQUESTOR_ID,
		sizeof(wl_nan_instance_id_t), &cmd_data->remote_id);
	if (unlikely(ret)) {
		goto fail;
	}
	...
	bcm_struct_cfgdev *cfgdev, const wl_event_msg_t *event, void *data)
{
	...
	case WL_NAN_EVENT_RECEIVE:
		ptr += sprintf(buf,
			...
			nan_event_str[event_num], tlv_data.inst_id,
	...
	case WL_NAN_EVENT_P2P:
		ptr += sprintf(buf, SUPP_EVENT_PREFIX"%s "
				MAC_ADDR_PREFIX MACF, nan_event_str[event_num],
				ETHER_TO_MACF(tlv_data.mac_addr));
		if (tlv_data.vend_info.data && tlv_data.vend_info.dlen) {
			...
			if (*(data + 6) == NAN_VENDOR_TYPE_P2P) {
				...
				ptr += sprintf(ptr, " %s", P2P_IE_PREFIX);
