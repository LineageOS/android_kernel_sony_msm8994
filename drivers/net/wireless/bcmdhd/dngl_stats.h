 ...
 *
 * $Id: dngl_stats.h 596029 2015-10-29 11:23:38Z $
 */
...
typedef struct {
	...
	u64 average_tsf_offset;		/* average beacon offset encountered (beacon_TSF - TBTT)
					 ...
					 */
	u32 leaky_ap_detected;		/* indicate that this AP typically leaks packets beyond
					 * the driver guard time.
					 */
	u32 leaky_ap_avg_num_frames_leaked;	/* average number of frame leaked by AP after
						 * frame with PM bit set was ACK'ed by AP
						 */
	u32 leaky_ap_guard_time;	/* guard time currently in force (when implementing IEEE
