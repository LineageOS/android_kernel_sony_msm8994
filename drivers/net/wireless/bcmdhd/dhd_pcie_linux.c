...
int dhdpcie_set_suspend_resume(struct pci_dev *pdev, bool state)
{
	...
	if (bus && (bus->suspended == TRUE) && (bus->host_suspend == TRUE)) {
		if (state == TRUE) {
			...
			if (!bus_wake(bus)) {
				DHD_ERROR(("%s: Failed to resume the bus.\n",
					__FUNCTION__));
				return -EAGAIN;
			} else {
				/* Carry on suspending */
				DHD_ERROR(("%s: Woke up from runtime suspend. "
					"Carry on legacy suspend.\n", __FUNCTION__));
				mutex_lock(&bus->pm_lock);
			}
		} else {
...
dhdpcie_start_host_pcieclock(dhd_bus_t *bus)
{
	...
	mutex_lock(&bus->host_clock_lock);
#ifdef CONFIG_ARCH_MSM
#ifdef SUPPORT_LINKDOWN_RECOVERY
	if (bus->islinkdown) {
...
#endif /* CONFIG_ARCH_MSM */
	mutex_unlock(&bus->host_clock_lock);
	DHD_TRACE(("%s Exit:\n", __FUNCTION__));
	return ret;
}
